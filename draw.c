...
void DrawBoard(int width, int height, struct board *board)
{
	...
	for (int i = 0; i < 9; i++)
	{
		for (int j = 0; j < 9; j++)
		{
			...
			{
				board->turn = 1;
				struct move botmove = botrandom(*board, 0);

				setpiece(board, i + j * 9, getpiece);			
			...
			if (collisionpointrect(GetMouseX(), GetMouseY(), posx + edgeGap + squareGap + squareDist * i, posy + edgeGap + squareGap + squareDist * j, squareSize, squareSize) == 1)
			{
				...
				if (IsMouseButtonPressed(0))
				{
					if (chosenx != -1 && choseny != -1)
					{
						...
						if (checklegal(*board, chosenx, choseny, i, j) == 1)
						{
...
int checklegal(struct board board, int fromx, int fromy, int tox, int toy)
{
	int notblocked = 1;
	for (int k = 0; k < 9; k++)
	{
		for (int l = 0; l < 9; l++)
		{
			if (k >= min(tox, fromx) && k <= max(tox, fromx) && l >= min(toy, fromy) && l <= max(toy, fromy))
			{
				if ((getpiece(board, k + l * 9) != 0) && !(k == fromx && l == fromy))
				{
					notblocked = 0;
				}
			}
	...
	if (((tox == 0 && toy == 0) || (tox == 8 && toy == 0) || (tox == 0 && toy == 8) || (tox == 8 && toy == 8) || (tox == 4 && toy == 4)) && getpiece(board, fromx + fromy * 9) != 0x3)
	{
		notblocked = 0;
	}
